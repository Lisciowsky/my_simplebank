...
func TestCreateAccountApi(t *testing.T) {
	testCases := []struct {
...

		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().CreateAccount(gomock.Any(), gomock.Eq(body)).
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			marshalBody, err := json.Marshal(tc.body)
			require.NoError(t, err)

			request, err := http.NewRequest(http.MethodPost, url, bytes.NewBuffer(marshalBody))
			require.NoError(t, err)
